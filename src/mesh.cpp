...
std::vector<std::pair<int, int>> convert_face_to_edges(std::vector<Vector3i> display_faces) {
    std::set<std::pair<int, int>> unique_edges;

    auto construct = [&](int a, int b) {
        int min_idx = std::min(a, b), max_idx = std::max(a, b);
        unique_edges.insert(std::pair<int, int>(min_idx, max_idx));
    };

    for (int i = 0; i < display_faces.size(); ++i) {
        const auto &face = display_faces[i];
        construct(face[0], face[1]);
        construct(face[1], face[2]);
        construct(face[2], face[0]);
    }
...
void Mesh::convert_obj_format_to_mesh() {
    ...
    std::for_each(this->display_vertices.begin(), this->display_vertices.end(), [this](const Vector3f &vertex) {
        static int id = 0;
        this->vertices.push_back(std::make_shared<Vertex>(vertex, id++));
    });

    // quick helper function to find the pair vertex to it's edge
    std::map<std::pair<Vertex *, Vertex *>, std::shared_ptr<Edge>> edge_maps;
    const auto find_edge = [&, this](Vertex *v1, Vertex *v2) {
        auto key = std::pair<Vertex *, Vertex *>((v1 > v2) ? v1 : v2, (v1 > v2) ? v2 : v1); // make the key is ordered
        if (edge_maps.find(key) == edge_maps.end()) {
            static int id = 0;
            auto edge = std::make_shared<Edge>(nullptr, id++);
            edge_maps[key] = edge;
            this->edges.push_back(edge);
            return edge;
        }
        return edge_maps[key];
    };

    const auto create_face = [this]() {
        this->faces.push_back(std::make_shared<Face>(this->faces.size()));
        return this->faces.back();
    };

    const auto create_half_edge = [this]() {
        this->half_edges.push_back(std::make_shared<HalfEdge>(this->half_edges.size()));
        return this->half_edges.back();
    };

    // loop all the faces
    for (const auto &obj_face: this->display_faces) {
        auto face = create_face();
        std::shared_ptr<HalfEdge> first = nullptr, prev = nullptr;

        for (int i = 0; i < 3; i++) {
            auto v1 = this->vertices[obj_face[i]];
            auto v2 = this->vertices[obj_face[(i + 1) % 3]];

            auto edge = find_edge(v1.get(), v2.get());
            auto half_edge = create_half_edge();

            half_edge->vertex = v1;
            half_edge->edge = edge;
            half_edge->face = face;
            half_edge->twin = edge->he;
            half_edge->prev = prev;
            ...
            if (edge->he) {
                edge->he->twin = half_edge;
            }

            edge->he = prev = half_edge;
        ...
        prev->next = first;
        first->prev = face->he = this->half_edges.back();
    }
...
int Mesh::compute_genus() {
    ...
    int vertices_num = this->vertices.size();
    int edges_num = this->edges.size();
    int faces_num = this->faces.size();

    genus = 1 - (vertices_num - edges_num + faces_num) / 2;
    return genus;
}
...
float Mesh::compute_average_degree() {
    ...
    for (const auto &vertex: this->vertices) {
        aver_deg += vertex->neighbor_vertices().size();
    }

    return aver_deg / this->vertices.size();
}
...
void Mesh::simplify(const float ratio) {
    ...
    std::priority_queue<std::shared_ptr<Edge>, std::vector<std::shared_ptr<Edge>>, Cmp> cost_min_heap{
            std::begin(this->edges), std::end(this->edges)};
